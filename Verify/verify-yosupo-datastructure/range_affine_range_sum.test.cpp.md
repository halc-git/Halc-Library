...
data:
  ...
  bundledCode: "#line 1 \"Verify/verify-yosupo-datastructure/range_affine_range_sum.test.cpp\"\
    ...
    \ << 1) + 1]);\n        }\n    }\n    inline T _get(int32_t pos) { return M::mapp(del[pos],\
    \ tree[pos]); }\n    void _calc(int32_t p) {\n        p >>= 1;\n        while\
    \ (p > 0) {\n            tree[p] = M::op(_get(p << 1), _get((p << 1) + 1));\n\
    \            p >>= 1;\n        }\n    }\n    inline void _del_segment(int32_t\
    \ p) {\n        del[p << 1] = M::comp(del[p], del[p << 1]);\n        del[(p <<\
    \ 1) + 1] = M::comp(del[p], del[(p << 1) + 1]);\n        tree[p] = _get(p);\n\
    \        del[p] = M::id;\n    }\n    void _delay(int32_t p) {\n        int32_t\
    \ length = 32 - std::countl_zero((uint32_t)p);\n        for (int32_t i = length\
    \ - 1; i >= 1; i--) {\n            _del_segment(p >> i);\n        }\n    }\n \
    ...
    \     int32_t dl = lf >> (std::countr_zero((uint32_t)lf));\n        int32_t dr\
    \ = ri >> (std::countr_zero((uint32_t)ri));\n        _delay(dl);\n        _delay(dr\
    \ - 1);\n        T rel = M::e;\n        T rer = M::e;\n        while (lf < ri)\
  ...
  requiredBy: []
  timestamp: '2024-05-13 22:31:21+09:00'
